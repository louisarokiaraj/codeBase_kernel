...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        return 0;*/
        dbg(DBG_PRINT,"(GRADING3D) inside addr_perm\n");
        vmarea_t *new_vmarea;
        int vmmapObj_prot=0;
        new_vmarea = vmmap_lookup((p->p_vmmap),ADDR_TO_PN(vaddr));
        if(new_vmarea !=NULL){
            dbg(DBG_PRINT,"(GRADING3D) The output of vmmap_lookup is not NULL\n");
            vmmapObj_prot = new_vmarea->vma_prot;
            if((perm & (vmmapObj_prot))){
                dbg(DBG_PRINT,"(GRADING3D) the obtained vmarea has the specified permission. Returning with success\n");
                return 1;
            }
        }
        dbg(DBG_PRINT,"(GRADING3D) the vmarea does not have specified permission\n");
        return 0;
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        return 0;*/
        dbg(DBG_PRINT,"(GRADING3D) inside range_perm\n");
        uint32_t virtual_startAddr = ADDR_TO_PN(avaddr);
        uint32_t virtual_endAddr = ADDR_TO_PN(((uint32_t)avaddr)+(len-1));
        int addr_perm_ret=0;
        while(virtual_startAddr<=virtual_endAddr){
            addr_perm_ret = addr_perm(p,PN_TO_ADDR(virtual_startAddr),perm);
            if(addr_perm_ret==0){
                dbg(DBG_PRINT,"(GRADING3D) the given address range does not have specified permission\n");
                return 0;
            }
            virtual_startAddr++;
        }
    dbg(DBG_PRINT,"(GRADING3D) the given address range has specified permission. returning with success. !!\n");
    return 1;
}
