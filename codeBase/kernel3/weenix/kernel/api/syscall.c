...
sys_read(read_args_t *arg)
{
        ...
        return -1;*/
        dbg(DBG_PRINT, "(GRADING3D) Inside sys_read\n");
        read_args_t read_arg;
        int err_code = 0;
        int read_data = 0;
        int to_read=0;
        size_t temp_bytes;
        int copyUser_ret = 0;
        copy_from_user(&read_arg, arg, sizeof(read_arg));
        void *temp_buf = page_alloc();

        temp_bytes = read_arg.nbytes;
        to_read = MIN(temp_bytes,PAGE_SIZE);
        read_data = do_read((read_arg.fd),temp_buf,to_read);
        if(read_data>=0){
            dbg(DBG_PRINT, "(GRADING3D)read successful \n");
            copy_to_user((read_arg.buf),temp_buf,read_data);

            page_free(temp_buf);
            dbg(DBG_PRINT, "(GRADING3D) returning from sys_read\n");
            return read_data;
        }else{
            dbg(DBG_PRINT, "(GRADING3D) read failed\n");
            page_free(temp_buf);
            curthr->kt_errno = -read_data;
            return -1;
        }
}
...
sys_write(write_args_t *arg)
{
        ...
        return -1;*/
        dbg(DBG_PRINT,"(GRADING3D),inside wite system call\n");
        write_args_t write_arg;
        int err_code = 0;
        int write_data;
        int to_write=0;
        int copyUser_ret=0;
        size_t temp_bytes;
        copy_from_user(&write_arg, arg, sizeof(write_arg));
        void *temp_buf = page_alloc();

        temp_bytes = write_arg.nbytes;
        to_write = MIN(temp_bytes,PAGE_SIZE);
        copy_from_user(temp_buf,write_arg.buf,to_write);

        write_data = do_write(write_arg.fd,temp_buf,to_write);
        page_free(temp_buf);
        if(write_data>=0){
            dbg(DBG_PRINT,"(GRADING3D), write successful\n");
            return write_data;
        }else{
            dbg(DBG_PRINT,"(GRADING3D),write failed\n");
            curthr->kt_errno = -write_data;
            return -1;
        }
}
...
sys_getdents(getdents_args_t *arg)
{
        ...
        return -1;*/
        dbg(DBG_PRINT, "(GRADING3D) Inside sys_getdents\n");
        getdents_args_t getdents_arg;
        int err_code = 0;
        int i,j=0;
        int count=0;
        char *uaddr = NULL;
        ...
        int dirent_ret = 0;
        int copyUser_ret=0;
        dirent_t temp_dir_ent;
        copy_from_user(&getdents_arg, arg, sizeof(getdents_arg));
        count = ((int)((getdents_arg.count)/sizeof(dirent_t)));
        for(i=count;i>0;i--){
            dirent_ret = do_getdent(getdents_arg.fd,&temp_dir_ent);
            if(dirent_ret<0){
                dbg(DBG_PRINT, "(GRADING3D) error in do_getdent\n");
                curthr->kt_errno = -dirent_ret;
                return -1;
            }else if(dirent_ret ==0){
                dbg(DBG_PRINT, "(GRADING3D) do_getdent successful\n");
                break;
            }
            dbg(DBG_PRINT,"(GRADING3D), continuing with do_getdent \n");
            uaddr = (char*)(getdents_arg.dirp) + (j*(sizeof(dirent_t)));
            copy_to_user(uaddr,&temp_dir_ent,sizeof(dirent_t));
            data_copied = data_copied + dirent_ret;
            j++;
        }
        dbg(DBG_PRINT,"(GRADING3D), returning from sys_getdents\n");
    return data_copied;
}
