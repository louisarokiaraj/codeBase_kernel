...
do_read(int fd, void *buf, size_t nbytes)
{
    ...
    int retval;
    if(fd<0 || fd>NFILES){
        dbg(DBG_PRINT, "(GRADING3D), Invalid File descriptor to read the file\n");
        return -EBADF;
    }if (curproc->p_files[fd]!=NULL){
        dbg(DBG_PRINT, "(GRADING3D), Checking file descriptor valid or not. It is VALID\n");
        f = fget(fd);
...
do_write(int fd, const void *buf, size_t nbytes)
{
    ...
    if(!(fd<0 || fd>NFILES)){
        dbg(DBG_PRINT, "(GRADING3D), Checking file descriptor valid or not. It is VALID\n");
        if(curproc->p_files[fd]!=NULL){
           dbg(DBG_PRINT, "(GRADING3D), File descriptor is VALID\n");
            f = fget(fd);
...
do_link(const char *from, const char *to)
{
       /* NOT_YET_IMPLEMENTED("VFS: do_link");
        return -1;*/
        dbg(DBG_PRINT, "(GRADING3D) inside do_link\n");
     if(strlen(from)> NAME_LEN ||strlen(to)> NAME_LEN){
        dbg(DBG_PRINT,"(GRADING3D), path name is too long in do_link\n");
        return -ENAMETOOLONG;
      }
      ...
      int ret_open = open_namev(from,flag,&res_vnode1,NULL);
      /*edit flag and base */

      if(S_ISDIR(res_vnode1->vn_mode))
      {
        dbg(DBG_PRINT, "(GRADING3D) the file is a directory\n");

        vput(res_vnode1);
        return -EISDIR;
      }

      int ret_dir = dir_namev(to,&namelen,&name,NULL,&res_vnode2);
      if(ret_dir == 0)
      {
        dbg(DBG_PRINT, "(GRADING3D) dir_namev successful\n");
        int ret_lookup = lookup(res_vnode2,name,namelen,&result);
        if(ret_lookup==0)
        {
          dbg(DBG_PRINT, "(GRADING3D) lookup successful so returning error\n");
          vput(res_vnode1);
          vput(res_vnode2);
          vput(result);
          return -EEXIST;
        ...
        {
          dbg(DBG_PRINT, "(GRADING3D) lookup not sucessful so calling link\n");
          int ret_link = res_vnode2->vn_ops->link(res_vnode1,res_vnode2,name,namelen);
          vput(res_vnode1);
          vput(res_vnode2);
          return ret_link;
        }
