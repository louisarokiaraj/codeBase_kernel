...
{
        KASSERT(file);
        dbg(DBG_PRINT, "(GRADING2A 1.a), Input File not equal to NULL\n");
        KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
        dbg(DBG_PRINT, "(GRADING2A 1.a), Input file is a special file\n");
        if (S_ISCHR(file->vn_mode)){
            KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
            dbg(DBG_PRINT, "(GRADING2A 1.a), Read function exists\n");
            int x = file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count);
            return x;
        }
        return -ENOTSUP;
}
...
{
        KASSERT(file);
        dbg(DBG_PRINT, "(GRADING2A 1.b), Input File not equal to NULL\n");
        KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
        dbg(DBG_PRINT, "(GRADING2A 1.b), Input file is a special file\n");
        if (S_ISCHR(file->vn_mode)){
            KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
            dbg(DBG_PRINT, "(GRADING2A 1.b), Write function exists\n");
            int x = file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count);
            return x;
        }
        return -ENOTSUP;
...
{
        dbg(DBG_PRINT, "(GRADING3D) Inside special_file_mmap\n");
        KASSERT(file);
        KASSERT(vma);
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->mmap);
        dbg(DBG_PRINT, "(GRADING3D) mmap not NULL\n");
