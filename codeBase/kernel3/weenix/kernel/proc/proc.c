...
proc_create(char *name)
{
        ...
        int i;
        for(i=0;i<NFILES;i++){
            dbg(DBG_PRINT, "(GRADAING2B) Initializing all the file Pointers\n");
            newProcess->p_files[i] = NULL;
        }if(curproc!=NULL){
            dbg(DBG_PRINT, "(GRADAING2B) Changing the current working directory for the newly created process\n");
            newProcess->p_cwd = curproc->p_cwd;
            if(curproc->p_cwd!=NULL){
                dbg(DBG_PRINT, "(GRADING2B), Incrementing reference count if parent process p_cwd is not null\n");
                vref(curproc->p_cwd);
            }
        }else{
                dbg(DBG_PRINT, "(GRADING2B), Setting the current working directory of new process back to NULL\n");
                newProcess->p_cwd = NULL;
        }
        newProcess->p_brk = NULL;
        newProcess->p_start_brk = NULL;
#ifdef __VM__
        dbg(DBG_PRINT, "Creating the vmmap obj for the new process\n");
...
proc_cleanup(int status)
{
     ...
     sched_wakeup_on(&(curproc->p_pproc->p_wait));
       for (j = 0;j<NFILES;j++) {
            dbg(DBG_PRINT, "(GRADING2B), Looping through the files in the current process to catch the NULL files.\n");
            if (curproc->p_files[j] != NULL) {
               dbg(DBG_PRINT, "(GRADING2B), Removing Files in the process\n");
               do_close(j);
            }
       }
       if (curproc->p_cwd != NULL) {
           dbg(DBG_PRINT, "(GRADING2B), Decrementing p_cwd reference count\n");
           vput(curproc->p_cwd);
       }
#if __VM__
       if (curproc->p_vmmap != NULL) {
            dbg(DBG_PRINT, "Destroyin the map\n");
...
do_waitpid(pid_t pid, int options, int *status)
{
        ...
        if(list_empty(&curproc->p_children)){
            ...
            while(!flag){
            if(pid == -1){
                    dbg(DBG_PRINT, "(GRADING1C 1) Scanning for any Dead child\n");
                    list_iterate_begin(&curproc->p_children,childProc,proc_t,p_child_link){
                ...
                if(flag==0){
                    dbg(DBG_PRINT, "(GRADING1C 1), thread is going to sleep, didnt find any active dead child\n");
                    sched_sleep_on(&(curproc->p_wait));
                }
            }else if(pid > 0){
                dbg(DBG_PRINT, "(GRADING1C 1) Scanning for Dead child with pid %d\n",pid);
                list_iterate_begin(&curproc->p_children,childProc,proc_t,p_child_link){
        ...
        END:
        dbg(DBG_PRINT, "(GRADING1C 1), Dead child cleaned up. do wait PID returning with terminated process %d\n",terminated_pid);
        return terminated_pid;
}
