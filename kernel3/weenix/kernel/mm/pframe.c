...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
    ...
    pframe_t *new_pframe_alloc=NULL;
    int pframe_ret=0;
    res_pframe = pframe_get_resident(o,pagenum);
    if(res_pframe){
        dbg(DBG_PRINT, "(GRADING3D) pframe get resident returned with pframe not null\n");
        GET_RESIDENT:
        if(pframe_is_busy(res_pframe)){
            dbg(DBG_PRINT, "(GRADING3D) returned pframe is busy. So sleeping\n");
            sched_sleep_on(&res_pframe->pf_waitq);
            goto GET_RESIDENT;
        }else{
            *result = res_pframe;
            KASSERT(NULL != *result);
            dbg(DBG_PRINT, "(GRADING3A 1.a) the returning pframe is VALID\n");
            KASSERT(!pframe_is_busy(*result));
            dbg(DBG_PRINT, "(GRADING3A 1.a) the returning result is not busy. Hence Returning\n");
            return 0;
        }
    }else if(res_pframe == NULL){
        new_pframe_alloc = pframe_alloc(o,pagenum);
        dbg(DBG_PRINT, "(GRADING3D) Newly allocating a pframe\n");
        if(new_pframe_alloc == NULL){
            dbg(DBG_PRINT, "(GRADING3D) No kernel memory to allocate a new pframe\n");
            return -ENOMEM;
        }
        dbg(DBG_PRINT, "(GRADING3D) Filling the newly created pframe\n");
        pframe_ret = pframe_fill(new_pframe_alloc);
        if(pframe_ret<0){
         dbg(DBG_PRINT, "(GRADING3D) pframe fill returned in error\n");
         pframe_free(new_pframe_alloc);
         return pframe_ret;
        }
        *result = new_pframe_alloc;
         KASSERT(NULL != *result);
         dbg(DBG_PRINT, "(GRADING3A 1.a) the returning filled pframe is VALID\n");
         KASSERT(!pframe_is_busy(*result));
         dbg(DBG_PRINT, "(GRADING3A 1.a) the returning result is not busy. Hence Returning\n");
        return 0;
    }
...
pframe_pin(pframe_t *pf)
{
        ...
        dbg(DBG_PRINT, "(GRADING3A 1.b) page is not free\n");
        KASSERT(pf->pf_pincount >= 0);
        dbg(DBG_PRINT, "(GRADING3A 1.b) page pin count is  > 0\n");
        if (!(pframe_is_pinned(pf))){
            dbg(DBG_PRINT, "(GRADING3D) page is not yet pinned, pin it\n");
            list_remove(&pf->pf_link);
            nallocated--;
        ...
        pf->pf_pincount++;
        dbg(DBG_PRINT, "(GRADING3D) increased pf->pf_pincount\n");
}
...
{
        KASSERT(!pframe_is_free(pf));
        dbg(DBG_PRINT, "(GRADING3A 1.c) page is not free\n");
        KASSERT(pf->pf_pincount > 0);
        dbg(DBG_PRINT, "(GRADING3A 1.c) page pin count is  > 0, decrement it\n");
        pf->pf_pincount--;
        if (!(pframe_is_pinned(pf))){
            dbg(DBG_PRINT, "(GRADING3D) page is not yet pinned\n");
            list_remove(&pf->pf_link);
            nallocated++;
            list_insert_tail(&alloc_list, &pf->pf_link);
            npinned--;
            dbg(DBG_PRINT, "(GRADING3D) page removed from alloc list and added to pinned list\n");
        }
}
