...
{
        dbg(DBG_PRINT, "(GRADING1C 1) Sleeping mode ON\n");
        KASSERT(q != NULL && curthr !=NULL);
        curthr->kt_state = KT_SLEEP;
        ktqueue_enqueue(q,curthr);
        sched_switch();
        dbg(DBG_PRINT, "(GRADING1C 1) Sleeping mode OFF\n");
}
...
sched_wakeup_on(ktqueue_t *q)
{
        ...
        if(!(sched_queue_empty(q))){
            ...
            dbg(DBG_PRINT, "(GRADING1C 3) Woke up thread of process with PID = %d\n",thread->kt_proc->p_pid);
            KASSERT(thread !=NULL);
            KASSERT(thread->kt_state == KT_SLEEP_CANCELLABLE || thread->kt_state == KT_SLEEP);
            dbg(DBG_PRINT, "(GRADING1A 4.a) Thread is in Cancellable sleep or Sleep state\n");
...
sched_broadcast_on(ktqueue_t *q)
{
        KASSERT(q !=NULL);
        while((q->tq_size)!=0){
            dbg(DBG_PRINT, "(GRADING1C 3) Trying to wake up all the thread in the queue\n");
            sched_wakeup_on(q);
        }  
}
...
{
        KASSERT(kthr != NULL);
        dbg(DBG_PRINT, "(GRADING1C 5), Asking the thread to Cancel by Making the kt_cancelled ON \n");
        kthr->kt_cancelled = 1;
        if(kthr->kt_state == KT_SLEEP_CANCELLABLE){
        dbg(DBG_PRINT, "(GRADING1C 5),Thread Removed from wait queue, thread's state is KT_SLEEP_CANCELLABLE\n");
...
sched_switch(void)
{
        kthread_t *newthr,*old_thread=NULL;
        uint8_t curipl = intr_getipl();
        curipl=intr_getipl();
        intr_setipl(IPL_HIGH);
        while(sched_queue_empty(&kt_runq)){
              dbg(DBG_PRINT, "(GRADING1C 1),Threads are waiting on the run queue for it's turn\n");
              intr_disable();
              intr_setipl(IPL_LOW);
              intr_wait();
        }
        ...
        dbg(DBG_PRINT, "(GRADING1C 1),Switching the thread to the next thread in the run queue\n");
        old_thread = curthr;
...
sched_make_runnable(kthread_t *thr)
{
        ...
        dbg(DBG_PRINT, "(GRADING1A 4.b) thread is not in the runq\n");
        thr->kt_state=KT_RUN;
        dbg(DBG_PRINT, "(GRADING1C 2) Making the thread of the process with PID %d Running\n",thr->kt_proc->p_pid);
